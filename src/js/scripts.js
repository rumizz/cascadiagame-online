...
function removeSoloTilesTokens() {
  if (turnsLeft == 1) {
    ...
    // setTimeout(function () {
    //   $(
    //     '.tokenTileContainer[tokentilenum="' + chosenTile + '"] .tileContainer'
    //   ).remove();
...
function pickNewTilesTokens() {
  ...
  for (let k = 0; k < newTileTokenContainerPositions.length; k++) {
    ...
    var thisTile = state.allTiles.splice(0, 1);
    ...
    var thisToken = state.allTokens.splice(0, 1);
  ...
  if (duplicateAmount == 4) {
    ...
    setTimeout(function () {
      ...
      for (let j = 0; j < 4; j++) {
        ...
        let newToken = state.allTokens.splice(0, 1);
      ...
      state.allTokens = shuffle(state.allTokens);
